...
inline geom_t euclid2(const Point& a, const Point& b) { //a와 b의 직선 거리
    Point ret = a - b;
    return ret.xx*ret.xx + ret.yy*ret.yy;
}
inline geom_t manhattan(const Point& a, const Point& b) { //a와 b의 택시 거리
    Point ret = a - b;
...
inline bool intersect(const Line &a, const Line &b){ //직선 a와 b가 교차? (정수)
    ...
    if(!ab && !cd) // c a-b d 혹은 a c-d b
        return (min(b.st, b.ed) <= max(a.st, a.ed)) && (min(a.st, a.ed) <= max(b.st, b.ed));
    return (ab <= 0) && (cd <= 0);
} 
...
inline int diff(double lhs, double rhs) { //실수 lhs < rhs
    if (lhs - EPS < rhs && rhs < lhs + EPS) return 0;
    return (lhs < rhs) ? -1 : 1;
}
inline bool is_between(double check, double a, double b) { //실수 a < check < b
